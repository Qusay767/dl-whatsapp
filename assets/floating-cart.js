...
function renderDrawer(body, data){
  ...
  if (!data.items || !data.items.length){
    body.append('<div class="dlwa-empty">Tu carrito está vacío.</div>');
...
function updateQty(key, qty){ return $.post(DLWA.ajax_url, { action:'dlwa_update_qty', nonce:DLWA.nonce, cart_item_key:key, qty:qty }); }
function removeItem(key){ return $.post(DLWA.ajax_url, { action:'dlwa_remove_item', nonce:DLWA.nonce, cart_item_key:key }); }
$(function(){
  ...
  $fab.on('click', function(){ openDrawer(); refreshDrawer().done(function(res){ if(res&&res.success){ renderDrawer($body,res.data); } }); });
